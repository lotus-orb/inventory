			            		...
			            		<div class="table-responsive">
				            		<table class="table table-bordered table-head-bg-info table-bordered-bd-info mt-4">
				            			...
				            			<tbody id="tab">
				            				...
				            				$c=0;
				            				@endphp
				            				@foreach($itemspk as $item)

							                    <tr id="row{{$i++}}" class="tab_added">
						                        	<td>
						                        		<input type="hidden" name= "category_id[]" value="{{$item->category->nm_kategori}}">
		...
		$(document).ready(function(){
			...
			$('#add').click(function(){
				...
				'<tr id="row'+i+'" class="tab_added">'+
					...
					'<td style="text-align: center;">' + '<input type="hidden" name= "satuan[]" value="' + sat_v + '">' + sat_t + '</td>'+
					'<td style="text-align: center;">' + '<input type="hidden" name= "jumlah[]" value="' + jumlah + '">' + jumlah + '</td>'+
					'<input type="hidden" name= "balance[]" value="0">'+
					'<td>'+
